...
app = FastAPI(title="Recaptcha Classification API", description="V3 EfficientNet-B1 Model Inference API")
...
def health_check():
    ...
    return {"status": status, "model_version": "v3", "architecture": "EfficientNet-B1"}
...
async def predict_image(file: UploadFile = File(...)):
    ...
    try:
        # 업로드된 파일 읽기 (Async I/O)
        content = await file.read()
        image = Image.open(BytesIO(content))
        
        # CPU/GPU 집약적인 추론 작업을 스레드풀에서 실행하여 이벤트 루프 차단 방지
        # 이렇게 하면 추론 중에도 다른 요청(Preflight, Health check 등)을 처리할 수 있습니다.
